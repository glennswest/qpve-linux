...
mkdir temp
cd temp
wget https://github.com/nmstate/nmstate/releases/download/v2.2.37/nmstatectl-linux-x64.zip
...
mkdir temp
cd temp
wget --no-check-certificate ${basepath}/openshift-install-linux.tar.gz -O openshift-install.tar.gz
tar xvzf openshift-install.tar.gz
mv openshift-install /usr/local/bin
wget --no-check-certificate ${basepath}/openshift-client-linux.tar.gz -O openshift-client.tar.gz
